...
class SwiftmailerExtension extends Extension
{
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if ('default' === $name && $config['default_mailer'] !== $name) {
            throw new InvalidArgumentException("Only default mailer can be named default");
        }

        $container->setAlias('mailer', 'swiftmailer.mailer.default');
        $container->setAlias('swiftmailer.mailer', 'swiftmailer.mailer.default');
        $container->setAlias('swiftmailer.transport', 'swiftmailer.mailer.default.transport');
        if ($container->has('swiftmailer.mailer.default.spool')) {
            $container->setAlias('swiftmailer.spool', 'swiftmailer.mailer.default.spool');
        }
        if ($container->has('swiftmailer.mailer.default.transport.real')) {
            $container->setAlias('swiftmailer.transport.real', 'swiftmailer.mailer.default.transport.real');
        }
    }
    ...
    protected function configureMailer($name, array $mailer, ContainerBuilder $container, $isDefaultMailer = false)
    {
        ...
        // alias
        if (true === $isDefaultMailer && 'default' != $name) {
            $container->setAlias('swiftmailer.mailer.default', sprintf('swiftmailer.mailer.%s', $name));
            $container->setAlias('swiftmailer.mailer.default.transport', sprintf('swiftmailer.mailer.%s.transport', $name));
            $container->setParameter('swiftmailer.mailer.default.spool.enabled', $container->getParameter(sprintf('swiftmailer.mailer.%s.spool.enabled', $name)));
            $container->setParameter('swiftmailer.mailer.default.delivery.enabled', $container->getParameter(sprintf('swiftmailer.mailer.%s.delivery.enabled', $name)));
            $container->setParameter('swiftmailer.mailer.default.single_address', $container->getParameter(sprintf('swiftmailer.mailer.%s.single_address', $name)));
        }
    }
    ...
    protected function configureMailerSpool($name, array $mailer, ContainerBuilder $container, $transport, $isDefaultMailer = false)
    {
        if (isset($mailer['spool'])) {
            ...
            if ($type === 'file') {
                ...
                if (!$isDefaultMailer) {
                    throw new InvalidArgumentException("Only default mailer can have a memory spool");
                }
                $container
            ...
            if (true === $isDefaultMailer && 'default' != $name) {
                $container->setAlias('swiftmailer.mailer.default.spool', sprintf('swiftmailer.mailer.%s.spool', $name));
                $container->setAlias('swiftmailer.mailer.default.transport.real', sprintf('swiftmailer.mailer.%s.transport.real', $name));
            }
        } else {
    ...
    protected function configureMailerSenderAddress($name, array $mailer, ContainerBuilder $container, $isDefaultMailer = false)
    {
        if (isset($mailer['sender_address']) && $mailer['sender_address']) {
            ...
            if (true === $isDefaultMailer && 'default' != $name) {
                $container->setAlias('swiftmailer.mailer.default.plugin.impersonate', sprintf('swiftmailer.mailer.%s.plugin.impersonate', $name));
            }
        } else {
    ...
    protected function configureMailerAntiFlood($name, array $mailer, ContainerBuilder $container, $isDefaultMailer = false)
    {
        if (isset($mailer['antiflood'])) {
            $container->setParameter(sprintf('swiftmailer.mailer.%s.plugin.antiflood.threshold', $name), $mailer['antiflood']['threshold']);
            $container->setParameter(sprintf('swiftmailer.mailer.%s.plugin.antiflood.sleep', $name), $mailer['antiflood']['sleep']);
            ...
            if (true === $isDefaultMailer && 'default' != $name) {
                $container->setAlias('swiftmailer.mailer.default.plugin.antiflood', sprintf('swiftmailer.mailer.%s.plugin.antiflood', $name));
            }
        }
    }
    ...
    protected function configureMailerDeliveryAddress($name, array $mailer, ContainerBuilder $container, $isDefaultMailer = false)
    {
        if (isset($mailer['delivery_address']) && $mailer['delivery_address']) {
            ...
            if (true === $isDefaultMailer && 'default' != $name) {
                $container->setAlias('swiftmailer.mailer.default.plugin.redirecting', sprintf('swiftmailer.mailer.%s.plugin.redirecting', $name));
            }
        } else {
    ...
    protected function configureMailerLogging($name, array $mailer, ContainerBuilder $container, $isDefaultMailer = false)
    {
        if ($mailer['logging']) {
            ...
            if (true === $isDefaultMailer && 'default' != $name) {
                $container->setAlias('swiftmailer.mailer.default.plugin.messagelogger', sprintf('swiftmailer.mailer.%s.plugin.messagelogger', $name));
            }
        }
    }
